...
set search_path = ''
as $$
  select nullif(btrim(coalesce(e.nume, '') || ' ' || coalesce(e.prenume, '')), '')
...
De ce textul pașilor se COPIAZĂ din șablon în loc să fie referit: un checklist finalizat este o dovadă în timp — la un control ITM sau la un litigiu de muncă, întrebarea este „ce i s-a cerut acestui om, în ziua aceea”. Dacă pasul ar fi doar un `template_item_id`, orice redenumire ulterioară a șablonului („Predare laptop” devine „Predare echipamente IT și revocare acces”) ar rescrie retroactiv sute de checklist-uri închise, iar dovada de parcurgere ar afirma altceva decât s-a bifat efectiv. Copierea rupe legătura: șablonul evoluează liber pentru viitor, instanțele rămân fixate în trecut. `template_item_id` se păstrează, dar doar informativ (`on delete set null`), pentru rapoarte de tipul „cât durează în medie pasul X”, nu ca sursă a textului. Trigger-ul `internal.checklist_pregateste_pasul()` refuză explicit modificarea `titlu`/`obligatoriu` pe o instanță, ca să nu se ocolească regula prin UPDATE direct.

De ce ciclul face parte din cheia unică: un fost angajat reangajat trebuie să parcurgă un al doilea onboarding, pe același șablon, în aceeași organizație. Cu `unique (organization_id, employee_id, template_id)` a doua angajare ar lovi o violare de unicitate, iar singurele ieșiri ar fi urâte: fie ștergerea istoricului primei integrări, fie duplicarea șablonului. Cu `ciclu` în cheie, cele două parcurgeri coexistă, se pot compara, iar `internal.checklist_pregateste_instanta()` alocă singur ciclul următor (`max(ciclu) + 1`), deci clientul nu trebuie să-l calculeze și nu îl poate coliziona. Același mecanism acoperă și cazul, mai rar, al unui onboarding reluat după o suspendare a contractului.

DEPENDENȚE DE VERIFICAT ÎNAINTE DE APLICARE (nu am putut citi migrările existente — folosire de tool-uri interzisă prin sarcină). Dacă vreun nume diferă, migrarea eșuează la creare:
1. `supabase/migrations/0010_inventory.sql` — am presupus coloanele `inventory_allocations(organization_id, inventory_item_id, employee_id, predat_la, returnat_la, deleted_at)` și `inventory_items(organization_id, denumire, cod)`. Codul „INV-0042” din mesaj vine din `inventory_items.cod`.
2. `public.employees(nume, prenume)` — folosite în `app.checklist_nume_angajat`; dacă există `nume_complet`, se înlocuiește expresia.
3. `public.employee_documents(id)` — țintă pentru `dovada_document_id`; `public.departments(id)`, `public.job_positions(id)`, `public.organizations(id)`.
4. `internal.set_actor()` — am presupus că populează `created_by`/`updated_by` (coloane prezente pe toate tabelele noi) și că se atașează `before insert or update ... for each row`.
5. Tipul enumerat al rolurilor este `public.app_role` (folosit la `responsabil_rol`); dacă trăiește în altă schemă, se ajustează calificarea.
6. Cele două politici adăugate pe `inventory_allocations` / `inventory_items` sunt o dependență inter-modul deliberată: fără ele, lista bunurilor nereturnate ar fi filtrată de RLS după drepturile pe Inventar ale celui care finalizează, iar un refuz incomplet ar lăsa laptopul plecat pe ușă. Dacă rolul care rulează funcțiile `SECURITY DEFINER` are deja BYPASSRLS, ele sunt redundante și pot fi șterse.
7. `verificare_automata` cunoaște trei valori, dar doar `inventar_returnat` are automatizare în această fază; `acces_revocat` și `documente_semnate` rămân pași manuali până când există sursele lor de adevăr.