...
SEMNALĂRI — lucruri pe care nu le-am putut verifica în inventar și care trebuie confirmate înainte de aplicare:

1. `internal.set_actor()` — am presupus că populează coloanele `created_by` / `updated_by` (uuid). Dacă folosește alte nume, coloanele din cele 6 tabele trebuie redenumite.
2. `public.leave_requests.status` — nu cunosc valorile enum-ului din 0009; sincronizarea acceptă defensiv `'aprobata' | 'aprobat' | 'approved'` prin cast la text. De redus la valoarea reală.
3. `public.leave_request_days` — am presupus coloanele `leave_request_id` și `data`; `employee_id` îl iau din `leave_requests`. De aliniat.
4. Distincția `concediu` vs `medical` la sincronizare: toate zilele sincronizate primesc `tip_zi = 'concediu'`, pentru că nu am în inventar un indicator pe `leave_types` (gen `este_medical` sau `cod`). De completat când se confirmă coloana.
5. `public.employees.department_id` — folosit la filtrarea aprobării în bloc pe departament.
6. `blocata_de`, `aprobat_de`, `approved_by`, `created_by`, `updated_by` sunt `uuid` fără FK, pentru că nu am confirmat dacă `public.profiles.id` este id-ul de utilizator auth. De adăugat FK dacă da.
7. **Expirables**: `overtime_compensation.termen_folosire` și `holiday_compensation.termen_acordare` sunt scadențe reale și ar trebui proiectate în `expirables` prin `internal.sync_expirable(...)`, cu `entity_type` nou (`overtime_compensation`, `holiday_compensation`), `kind` distinct, plus înregistrare în `app.poate_vedea_expirabil()`. Nu am inclus triggerele pentru că nu am putut citi semnătura reală din `0008_expirables.sql` — de adăugat într-un follow-up scurt, nu de inventat aici.
8. Dacă există deja un `internal.set_updated_at()` generic, `internal.pontaj_marcheaza_actualizarea()` trebuie înlocuit cu el (l-am scris local ca să nu depind de un simbol neconfirmat).