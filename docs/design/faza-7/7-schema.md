...
Cum se calculează următoarea scadență de instruire: la salvarea unei instruiri, triggerul `internal.ssm_training_calc` citește periodicitatea în vigoare pentru tipul respectiv la data instruirii (`app.periodicitate_instruire` → rândul din `ssm_training_type_periods` cu cel mai recent `valabil_de_la` anterior datei) și adaugă acele luni la `data_instruirii`. Dacă tipul nu are periodicitate (introductiv-general, la locul de muncă, suplimentar) scadența rămâne nulă — instruirea nu se repetă calendaristic. Apoi `internal.ssm_training_sync` proiectează în `expirables` doar cea mai recentă instruire per (angajat, tip), cu `entity_id = employee_id` și `kind = domeniu:cod`, ca o instruire nouă să suprascrie scadența veche în loc să acumuleze rânduri. Modificarea unei periodicități se face adăugând un rând nou cu alt `valabil_de_la`, deci istoricul rămâne auditabil, iar instruirile vechi își păstrează scadența calculată la momentul lor.

De ce PSI este separat de SSM: sunt două obligații legale distincte, cu temeiuri diferite (Legea 319/2006 pentru securitate și sănătate în muncă, Legea 307/2006 și OMAI 712/2005 pentru apărarea împotriva incendiilor), cu tematici, lectori, periodicități și registre proprii, controlate de instituții diferite (ITM, respectiv ISU). O firmă cu instruirea SSM la zi poate fi complet neconformă pe PSI. De aceea `domeniu` este o coloană a tipului de instruire, iar `kind`-ul din `expirables` include domeniul: scadența PSI nu se poate ascunde niciodată în spatele celei SSM. Aceeași logică se aplică stingătoarelor, unde verificarea anuală, reîncărcarea și proba de presiune sunt trei `kind`-uri separate pe același `entity_id`.

Ce registre cere ITM la un control: registrul de evidență a instruirilor (fișe individuale de instruire, cu semnătură), evaluarea riscurilor pe fiecare loc de muncă și planul de prevenire și protecție cu termene și responsabili, registrul de evidență a accidentelor de muncă și dovada comunicării în termen la ITM, registrul de evidență a incidentelor periculoase, registrul de evidență a bolilor profesionale, procesele-verbale ale CSSM (peste pragul configurabil de lucrători), fișele de aptitudine de la medicina muncii, evidența EIP predat cu semnătură de primire, autorizațiile nominale ale personalului (stivuitorist, macaragiu, fochist, electrician) și autorizațiile ISCIR ale echipamentelor. Fiecare dintre acestea are o tabelă dedicată aici, cu soft delete și jurnal de audit, tocmai pentru ca reconstituirea la o dată din trecut să fie posibilă.

SEMNALĂRI (lucruri pe care nu le-am putut verifica, fiind read-only):
1. `app.write_audit(...)` — nu îi cunosc semnătura, așa că derogarea ISCIR nu o apelează direct; motivul, autorul și momentul derogării se scriu în coloanele `equipment.derogare_motiv/_acordata_de/_acordata_la`, iar triggerul atașat prin `internal.attach_audit('equipment')` le duce în `audit_logs`. Dacă `write_audit` are o semnătură stabilă, aici e locul unde ar trebui apelată explicit, cu `action = 'permission_changed'` sau un tip dedicat.
2. `internal.set_actor()` — am presupus că este o funcție de trigger fără argumente care completează `created_by/updated_by/updated_at`. Dacă cere argumente, bucla de la punctul 11 trebuie ajustată.
3. `public.app_role` și `public.permission_scope` — am presupus schema `public` (consistent cu `Database['public']['Enums']`). Singurul loc unde contează este castul `array[...]::public.app_role[]` din `internal.ssm_iscir_guard`.
4. `app.poate_vedea_expirabil` — am rescris-o integral, dar predicatele exacte ale ramurilor existente (0008) nu le-am putut citi; înainte de aplicare trebuie comparate rând cu rând cu originalul, altfel se pot relaxa sau strânge accidental drepturile pe tipurile vechi.
5. Coloanele reale ale tabelelor `employees`, `departments`, `job_positions`, `organizations` — am folosit doar `id`. Dacă `employees` are o cheie primară compusă sau altă denumire, cheile străine trebuie ajustate.
6. `app.seed_ssm_defaults` este SECURITY DEFINER și scrie în tabele cu FORCE RLS; funcționează dacă proprietarul funcției are BYPASSRLS (rolul `postgres` din Supabase). Dacă nu, triggerul `organizations_ssm_seed` trebuie mutat într-o acțiune de platformă sau politicile de INSERT pe `ssm_training_types` / `ssm_legal_parameters` trebuie să primească o ramură `app.is_service_context()`.
7. Ștergerea logică (setarea `deleted_at`) trece azi prin permisiunea `update`; dacă vrei să ceară `delete`, adaugă o politică UPDATE separată sau un trigger care verifică `app.can(..., 'delete', ...)` când `deleted_at` trece din null în non-null.
8. `occupational_diseases` conține date de sănătate (art. 9 GDPR) și este acum vizibilă de la scope `team` în sus, ca orice registru de organizație; dacă vrei restricționare la `all`, tabela trebuie scoasă din bucla generică și tratată cu politici proprii.
9. Suspendarea sau expirarea unei autorizații nominale nu retrage automat responsabilul de pe echipamentele ISCIR deja salvate — regula se aplică la scriere. Ar fi nevoie de un job zilnic care marchează echipamentele rămase fără responsabil autorizat.