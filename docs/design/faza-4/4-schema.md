...
create policy expirabile_update on public.expirables
  ...
  using (
    ...
    and (
      (select app.is_service_context())
      or app.has_permission(organization_id, 'compliance:update', 'team')
    )
  )
  with check (
    deleted_at is null
...
1. `app.has_permission(uuid, text, text)` — am presupus `(organization_id, permission_key, min_scope)`, adică `app.has_permission(org, 'compliance:read', 'team')`. Dacă semnătura reală este `(organization_id, resursa, actiune)`, toate politicile RLS din secţiunea 7 trebuie rescrise ca `app.has_permission(org, 'compliance', 'read')` şi diferenţierea de scope mutată într-un helper separat.
2. `app.is_manager_of(uuid, uuid)` — am presupus `(organization_id, employee_id)` cu sensul „utilizatorul curent este managerul angajatului". Dacă primul parametru este `manager_employee_id`, apelul din `expirabile_select` trebuie inversat.
3. Chei străine presupuse: `public.profiles(id)`, `public.employees(id)`, `public.notifications(id)`, `public.email_log(id)`, `public.organizations(id)` — toate cu PK numit `id`. Dacă `notifications` sau `email_log` au altă cheie primară, scoate cele două FK-uri (coloanele pot rămâne simple `uuid`).
4. `internal.seteaza_updated_at()` — dacă în migrările 0001..0005 există deja un helper echivalent (ex. `internal.set_updated_at`), şterge definiţia mea şi refoloseşte-l pe cel existent; am evitat `create or replace` peste un nume probabil ocupat.
5. `app.azi_local()` foloseşte constanta `'Europe/Bucharest'`. Dacă `organizations` are coloana `timezone` (există `Tenant.timezone` în TS), funcţia ar trebui să primească `p_organization_id` şi să citească fusul de acolo — am izolat expresia într-un singur loc tocmai pentru asta.
6. Cheile de permisiune `compliance:read`, `compliance:create`, `compliance:update` trebuie să existe în seed-ul `role_permissions`; resursa `compliance` apare în inventar, dar nu am putut verifica ce acţiuni sunt semănate.