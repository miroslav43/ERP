...
const CAMPURI_SENSIBILE = /^(cnp|iban|salariu|salariu_brut|salariu_net|suma_neta|motiv_medical|numar_certificat|serie_certificat|parola|password|token|hash|diagnostic)$/i;
...
if (DOAR_DIFF) {
  ...
  try {
    modificate = execSync("git diff --name-only HEAD", { cwd: RADACINA, encoding: "utf8" })
      .split("\n").filter(Boolean).map((f) => join(RADACINA, f));
  } catch { /* fără git: auditează tot */ }
...
const FEATURE_KEYS = (() => {
  ...
  return new Set([...ts.slice(start, ts.indexOf("] as const", start)).matchAll(/"([a-z_]+)"/g)].map((m) => m[1]));
})();
...
function blocuri(sursa) {
  ...
  for (const m of sursa.matchAll(/create(?:Platform|Public)?Action\s*(?:<[^>]*>)?\s*\(\s*\{/g)) {
    let i = m.index + m[0].length - 1, adanc = 0;
    for (; i < sursa.length; i++) {
      if (sursa[i] === "{") adanc++;
      else if (sursa[i] === "}") { adanc--; if (adanc === 0) break; }
    }
...
for (const f of fisiere) {
  ...
  for (const b of blocuri(sursa)) {
    ...
    liniiBloc.forEach((l, i) => {
      ...
      const tranzitie = /\b(status|aprobat|approved_at|approved_by|respins|blocat|blocata_la|anulat|finalizata_la|confirmat)\b/.test(fereastra);
      // Clientul admin ocolește RLS, deci un refuz tăcut din politică e improbabil.
      const clientAdmin = /\b(admin|createAdminSupabase\(\))\s*$|\badmin\b\s*\n?\s*\.from\(/.test(liniiBloc.slice(Math.max(0, i - 3), i + 1).join("\n"));
      adauga(tranzitie && !clientAdmin ? "EROARE" : "AVERT", "A4", f, linieBloc + i,
...
if (CA_JSON) console.log(JSON.stringify({ fisiere: fisiere.length, constatari }, null, 2));
  ...
  for (const c of [...erori, ...averturi]) console.log(`${c.nivel.padEnd(6)} ${c.regula}  ${c.unde}\n         ${c.mesaj}`);
