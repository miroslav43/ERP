...
let ev;
try { ev = JSON.parse(readFileSync(0, "utf8")); } catch { iesiCurat(); }
...
const migrari = () => readdirSync(DIR_MIGRARI).filter((f) => f.endsWith(".sql")).sort();
...
if (rel.startsWith("src/") && /^[^/*]*\.upsert\(/m.test(text)) {
  const partiale = new Map(), pline = new Map();
  ...
  for (const f of migrari()) {
    ...
    for (const m of sql.matchAll(
      ...
      const coloane = m[3].split(",").map((c) => c.trim().replace(/\s+(asc|desc)$/i, "")).sort().join(",");
  ...
  linii.forEach((linie, i) => {
    ...
    if (tabela === null) {
      avertismente.push(`\`.upsert(\` la linia ${i + 1}: n-am putut identifica tabela. Verifică manual dacă indexul unic e PARȚIAL (\`where deleted_at is null\`) — PostgREST nu emite predicatul în ON CONFLICT ⇒ 42P10 (capcana 7).`);
      return;
    }
    if (!mc) return;   // fără onConflict, PostgREST cade pe cheia primară
    const cerute = mc[1].split(",").map((c) => c.trim()).sort().join(",");
    const partial = (partiale.get(tabela) ?? []).find((ix) => ix.coloane === cerute);
    const plin = (pline.get(tabela) ?? []).find((ix) => ix.coloane === cerute);
    if (partial) {
      avertismente.push(`42P10 GARANTAT (capcana 7): \`.upsert\` pe \`${tabela}\` cu onConflict "${mc[1]}" țintește indexul PARȚIAL \`${partial.nume}\` (${partial.predicat}). Postgres respinge inferența la PLANIFICARE, deci pică la FIECARE apel, nu doar la conflict. Folosește citire-apoi-INSERT-sau-UPDATE (vezi src/app/(app)/pontaj/actions.ts).`);
    } else if (!plin) {
      avertismente.push(`\`.upsert\` pe \`${tabela}\` cu onConflict "${mc[1]}": n-am găsit niciun index unic care să acopere exact aceste coloane în supabase/migrations/. Fără index potrivit, Postgres dă 42P10.`);
    }
  });
}
...
if (/\.rpc\(\s*["']/.test(text)) {
  const inApp = new Set(), inPublic = new Set();
  for (const f of migrari()) {
    const sql = readFileSync(join(DIR_MIGRARI, f), "utf8");
    for (const m of sql.matchAll(/create\s+(?:or\s+replace\s+)?function\s+app\.(\w+)/gi)) inApp.add(m[1]);
    for (const m of sql.matchAll(/create\s+(?:or\s+replace\s+)?function\s+public\.(\w+)/gi)) inPublic.add(m[1]);
  }
  for (const m of text.matchAll(/\.rpc\(\s*["'](\w+)["']/g)) {
    if (inApp.has(m[1]) && !inPublic.has(m[1])) {
      avertismente.push(`\`.rpc("${m[1]}")\` — \`app.${m[1]}\` trăiește în schema \`app\`, iar supabase/config.toml expune la PostgREST doar ["public","graphql_public"]. Funcția NU e apelabilă și NU apare în Database["public"]["Functions"]: cod care nici nu compilează, nici nu rulează. Portează logica în TypeScript (capcanele 1 și 14).`);
    }
  }
}
