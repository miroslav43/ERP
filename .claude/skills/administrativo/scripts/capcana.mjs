...
if (!existsSync(DOC)) {
  console.error(
    `capcana: nu găsesc ${DOC}. Rulează din repo-ul Administrativo sau setează CLAUDE_PROJECT_DIR.`,
  );
  process.exit(3);
}
...
for (const m of brut.matchAll(/^(\d+)\.\s([\s\S]*?)(?=\n\n\d+\.\s|\n*$)/gm)) {
  ...
  capcane.push({
    ...
    tabele: [
      ...new Set(
        [...text.matchAll(/\b(?:public\.)?([a-z][a-z0-9]*(?:_[a-z0-9]+){0,4})\b/g)]
          .map((x) => x[1])
          .filter((t) => SUFIXE_TABELA.test(t) || TABELE_SIMPLE.has(t)),
      ),
    ],
...
const pozitional = argv.find(
  (a) => !a.startsWith("--") && argv[argv.indexOf(a) - 1]?.startsWith("--") !== true,
);

let rezultate = capcane,
  titlu = "toate capcanele";
const nr = optie("--nr");
...
if (nr) {
  rezultate = capcane.filter((c) => c.nr === Number(nr));
  titlu = `capcana ${nr}`;
} else if (tabela) {
  rezultate = capcane.filter((c) => c.tabele.includes(tabela));
  titlu = `tabela ${tabela}`;
} else if (modul) {
  rezultate = capcane.filter((c) => c.module.includes(modul));
  titlu = `modulul ${modul}`;
} else if (rol) {
  rezultate = capcane.filter((c) => c.roluri.includes(rol));
  titlu = `rolul ${rol}`;
} else if (argv.includes("--tacute")) {
  rezultate = capcane.filter((c) => c.tacut);
  titlu = "capcane TĂCUTE (fără cod de eroare)";
} else if (pozitional) {
...
console.log(
  `capcane.md — ${capcane.length} capcane · filtru: ${titlu} · ${rezultate.length} rezultate\n`,
);
for (const c of rezultate) {
  const et = [c.coduri.join(" "), c.tacut ? "TĂCUTĂ" : "", c.rezolvata ? "REZOLVATĂ" : ""]
    .filter(Boolean)
    .join(" · ");
...
if (rezultate.length === 0)
  console.log("   Nicio capcană. Asta NU înseamnă că e sigur — citește documentul.");
process.exit(0);
