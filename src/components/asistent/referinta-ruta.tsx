 ...
 */
function pictograma(destinatie: Destinatie): LucideIcon {
  ...
  if (destinatie.id.startsWith("fisa.")) return FileUser;
  const surse = destinatie.zona === "app" ? NAV_ITEMS : PORTAL_NAV_ITEMS;
  return surse.find((item) => item.id === destinatie.parinte)?.icon ?? ArrowRight;
}
...
export function ReferintaRuta({
  ...
  const Pictograma = pictograma(destinatie);
