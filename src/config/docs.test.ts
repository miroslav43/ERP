...
describe("capcane.md — numerotare și lizibilitate", () => {
  ...
  it("nu conține octeți NUL — altfel devine invizibil pentru grep", () => {
    ...
    expect(brut.includes(0), "capcane.md conține un octet NUL: scrie-l ca secvență de evadare").toBe(false);
  });
...
describe("căile citate în documentație există", () => {
  ...
  it("fiecare cale relativă din CLAUDE.md și .claude/**/*.md se rezolvă pe disc", () => {
    ...
    const tipar = /`((?:src|docs|supabase|tests|scripts|ops|\.claude)\/[A-Za-z0-9_./()[\]-]+\.(?:ts|tsx|sql|md|mjs|sh|json))`/g;
    const lipsa: string[] = [];
    for (const doc of documente()) {
      for (const m of citeste(doc).matchAll(tipar)) {
        ...
        if (!existsSync(join(RADACINA, cale))) lipsa.push(`${doc.replace(RADACINA + "/", "")} → ${cale}`);
      }
    }
...
describe("permisiuni — literalele din cod sunt chei reale", () => {
  it("fiecare literal pasat lui can()/scopeFor() e o cheie din PERMISSION_KEYS", () => {
      ...
      for (const intrare of readdirSync(dir)) {
        ...
        if (statSync(cale).isDirectory()) parcurge(cale);
          ...
          citeste(cale)
            .split("\n")
            .forEach((linie, i) => {
              for (const m of linie.matchAll(/\b(?:can|scopeFor)\s*\(\s*[A-Za-z_$][\w.$]*\s*,\s*"([a-z_]+:[a-z_]+)"/g)) {
                const cheie = m[1] as string;
                if (!gasite.has(cheie)) gasite.set(cheie, `${cale.replace(RADACINA + "/", "")}:${i + 1}`);
              }
            });
        }
      }
    ...
    expect(gasite.size, "n-am găsit niciun apel can() — expresia regulată a ruginit").toBeGreaterThan(20);
...
describe("CLAUDE.md descrie verificarea reală", () => {
  it("comanda prescrisă e derivabilă din scripturile package.json", () => {
    ...
    expect(verify.includes("build"), "package.json: `verify` include acum build — actualizează CLAUDE.md").toBe(false);
    const claude = citeste(CLAUDE_MD);
    expect(claude, "CLAUDE.md nu mai prescrie lanțul complet cu build").toContain(
      "pnpm typecheck && pnpm lint && pnpm test && pnpm build",
    );
    expect(claude, "CLAUDE.md nu mai avertizează că `verify` sare build-ul").toMatch(/NU include `?build`?/);
  });
});
